...
\footnote{\url{https://code.montera34.com:4443/numeroteca/covid19/-/blob/master/README_providencialdata19.md}}.
This information is compiled from several sources, mainly the official
web pages of the Spanish regional goverments. In addition, we consider
two sets of explanatory variables. The first one, and the focus of this
research, is set of two environmental variables, namely temperature and
humidity. The second set provides some relevant controls for
multivariate analysis, and refers to economic and demographic attributes
of the province. Table \ref{tab:descriptive-statistics} shows the
descriptive statistics and the provencance of the data.
...
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{l>{\raggedright\arraybackslash}p{15em}rrrr>{\raggedright\arraybackslash}p{6em}}
\toprule
Variable & Note & Min & Mean & Max & SD & Source\\
\midrule
\rowcolor{gray!6}  SARS-19 Incidence & Incidence in reported cases of SARS-19 per 100,000 people & 0.38 & 153.61 & 1149.36 & 186.23 & Datalista\\
...
without spatial effects (from here, SUR-SIM). The classical expresion
for this model in stacked form is,
...
the more relevant characteristic in SUR model,is the dependence among
error vector,
...
Note that this especification is highly flexible, allowing changes in
the coefficients \(\beta_{it}\) in order to modulate the effect of
\(X^i_{.t}\) on \(y_t\). This flexibility can be reduced and it is
posible to impose restrinctions considerong time constant some \(\beta\)
coefficients. In this case, we can reformulate the coefficients
expression of
\(\beta_t=(\beta_{1},...,\beta_{r},\beta_{r+1},...,\beta_{Nt})\)
...
autocorrelation in the residuals of (\ref{eq:sur-sim}) and several
Lagrange Multipliers tests has been develop to test the null of spatial
independence (see López et al. (2014)). In case of reject the null
alternative several specifications has been propose to include spatial
effects (Anselin (1988), see also Anselin (2016)). In this paper we
consider the specific spatial SUR model tahr incorpore a spatial lag of
the dependent variable as an explanatory factor. The stack expresion for
the spatial lag SUR model (SUR-SLM),
...
The SUR-SLM model can be estimated by maximum likelihood (López et al.
(2014)) or instrumental variables (Mínguez et al. (2019)). We considerer
this methodology to estimate the model.
