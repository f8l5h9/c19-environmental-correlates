...
Calculate density variable and join to `covid19_spain`: 
```{r}
covid19_spain <- covid19_spain %>%
  left_join(provinces_spain %>% 
              left_join(covid19_spain %>% 
                        ...
                        by = "ID_INE") %>%
              transmute(ID_INE, Density = Population/units::set_units(st_area(provinces_spain), km^2)) %>%
              st_drop_geometry(),
            by = "ID_INE")
```
...
week11.plot <- covid19_spain %>% 
  left_join(provinces_spain, by = c("province", "CCAA", "ID_INE")) %>%
  st_as_sf() %>%
...
covid19_spain %>% filter(Date == "2020-03-13") %>% 
  filter(CCAA != "Canarias") %>%
  left_join(provinces_spain, by = c("province", "ID_INE")) %>%
  st_as_sf() %>%  ggplot() +
