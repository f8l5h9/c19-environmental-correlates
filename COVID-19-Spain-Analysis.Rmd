...
data.frame(R2_lag8 = sur.slm_lag8$R2, 
  ...
  geom_point(size = 2) +
  scale_color_manual(values = c("R2_lag11" = "orange", "R2_lag8" = "green") ) + 
  theme_tufte()
...
Extract residuals for March 14 and April 2, and compute autocorrelated residuals on March 14:
```{r}
residuals_Mar14 <- as.matrix(residuals(sur.slm_lag11))[[1]]
residuals_Mar14 <- lag.listw(listw, residuals_Mar14)

residuals_Apr02 <- as.matrix(residuals(sur.slm_lag11))[[20]]
```
...
Plot residuals on April 2 (delta is not significant):
```{r}
covid19_spain %>% filter(Date == "2020-03-14") %>% 
  mutate(ResApr02 = residuals_Apr02) %>% 
  filter(CCAA != "Canarias") %>%
  ggplot() +
...
data.frame(Date = seq(ymd("2020-03-14"), 
                        ymd("2020-04-04"), 
                            by = "days"),
           Beta = matrix(sur.slm_lag11$coefficients,ncol = 22)[n,],
           tvalue = matrix(sur.slm_lag11$coefficients/sur.slm_lag11$rest.se,ncol = 22)[n,]) %>%
  mutate(sig = ifelse(abs(tvalue) > 1.64, "Significant at 5%", "Not Significant at 5%")) %>%
